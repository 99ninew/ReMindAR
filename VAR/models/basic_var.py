...
from VAR.models.helpers import DropPath, drop_path
...
# try:
#     from torch.nn.functional import scaled_dot_product_attention as _torch_sdp_attn
#     def slow_attn(query, key, value, scale: float, attn_mask=None, dropout_p=0.0):
#         # PyTorch version does NOT accept `scale` directly, so apply it manually
#         query = query * scale
#         return _torch_sdp_attn(query, key, value, attn_mask, dropout_p=dropout_p, is_causal=False)
except ImportError:
    def slow_attn(query, key, value, scale: float, attn_mask=None, dropout_p=0.0):
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, attn_bias):
        ...
        if using_flash:
            ...
            # q = q * self.scale
            # oup = slow_attn(query=q, key=k, value=v, attn_mask=attn_bias, dropout_p=dropout_p).transpose(1, 2).reshape(B, L, C)
