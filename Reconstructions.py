...
# only low-level model: python Reconstructions.py --subj=1 --model_name=prior_257_final_subj01_bimixco_softclip_byol --autoencoder_name=autoencoder_VAR_v3 --recons_per_sample=16 --img2img_strength=0
...
for val_i, (voxel, img, coco) in enumerate(tqdm(val_dl,total=len(ind_include))):
    ...
    with torch.no_grad():
        if img2img:
            ...
            blurry_recons = autoencoder.decode(image_enc_pred_trimmed, st=start_scale)

            if isinstance(blurry_recons, PIL.Image.Image):
                blurry_recons = transforms.ToTensor()(blurry_recons).unsqueeze(0)
            elif isinstance(blurry_recons, list) and isinstance(blurry_recons[0], PIL.Image.Image):
                blurry_recons = torch.stack([transforms.ToTensor()(img) for img in blurry_recons], dim=0)

            if val_i==0:
                if isinstance(blurry_recons, torch.Tensor):
                    img_show = utils.torch_to_Image(blurry_recons)
                else:
                    img_show = blurry_recons
                plt.imshow(img_show)
                plt.show()
        else:
            blurry_recons = None
...
if saving:
    ...
    torch.save(all_brain_recons,f'{model_name}_recons_img2img{img2img_strength}_{recons_per_sample}samples_st{start_scale}_lookup_epoch180_{autoencoder_name}.pt')
print(f'recon_path: {model_name}_recons_img2img{img2img_strength}_{recons_per_sample}samples_st{start_scale}_lookup_epoch180_{autoencoder_name}.pt')
