...
weights = torch.tensor([81, 36, 16, 9, 4, 1], dtype=torch.float32)
# weights = torch.tensor([9, 4, 1], dtype=torch.float32)
# weights = torch.tensor(weights/weights.sum()).to(device)
...
if voxel_dims == 1: # 1D data
    ...
    voxel2sd = Voxel2StableDiffusionModel(use_cont=use_cont, in_dim=in_dims[subj_id], ups_mode=ups_mode, st=start_scale)
...
train_url = f"{{{os.path.join(parent_dir, f'dataset/train/train_subj{subj_id}_{{0..17}}.tar')},{os.path.join(parent_dir, f'dataset/val/val_subj{subj_id}_0.tar')}}}" 
# f"{{/fsx/proj-fmri/shared/natural-scenes-dataset/webdataset_avg_split/train/train_subj{subj_id}_{{0..17}}.tar,/fsx/proj-fmri/shared/natural-scenes-dataset/webdataset_avg_split/val/val_subj{subj_id}_0.tar}}"
val_url = os.path.join(parent_dir, f"dataset/test/test_subj{subj_id}_{{0..1}}.tar")
# f"/fsx/proj-fmri/shared/natural-scenes-dataset/webdataset_avg_split/test/test_subj{subj_id}_{{0..1}}.tar"
meta_url = os.path.join(parent_dir, f"dataset/metadata_subj{subj_id}.json")
# f"/fsx/proj-fmri/shared/natural-scenes-dataset/webdataset_avg_split/metadata_subj{subj_id}.json"
...
lr_scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=max_lr, 
                                            total_steps=num_epochs*((num_train//batch_size)//num_devices), # 120*(8859//8=1107=steps_per_epoch)//1=132,840
                                            final_div_factor=1000,
                                            last_epoch=-1, pct_start=2/num_epochs) #warmup_percentage

# if local_rank == 0:
#     wandb.init(
#         project="voxel2sd",        
#         name=f"subj_{subj_id}",    
...
for epoch in progress_bar:
    ...
    for train_i, (voxel, image, _) in enumerate(train_dl):
        ...
        with torch.cuda.amp.autocast(enabled=use_mp):
            ...
            # print(autoenc_image.shape)
            image_enc = autoencoder.encode(autoenc_image, st=start_scale) 
            ...
            # print(len(image_enc_pred), len(image_enc))
            # for i in range(len(image_enc_pred)):
            #     print(image_enc_pred[i].shape, image_enc[i].shape)
            ...
            if use_cont:
                ...
                print(cnx_embeds.shape, cnx_aug_embeds.shape)
                print(transformer_feats.shape)
            ...
            # print("pred:", image_enc_pred[0],"\n image: ", image_enc[0])
            mse_loss = 0.0
            for i in range(len(image_enc_pred)):
                mse = F.mse_loss(image_enc_pred[i], image_enc[i])
                mse_loss += weights[i] * mse 
            
            # print("mse_loss", mse_loss.item())
            
            del image_512, voxel

            if use_reconst: #epoch >= 0.1 * num_epochs: # default = false
            ...
            loss = mse_loss + 2*reconst_loss + 0.1*cont_loss + 16*sobel_loss
    ...
    if local_rank==0: 
        ...
        for val_i, (voxel, image, _) in enumerate(val_dl): 
            with torch.inference_mode():
                ...
                with torch.cuda.amp.autocast(enabled=use_mp):
                    image_enc = autoencoder.encode(image, st=start_scale)
        ...
        if (not save_at_end and ckpt_saving) or (save_at_end and epoch == num_epochs - 1):
            ...
            try:
                ...
                if reconst is None:
                    ...
                    image_enc_pred_trimmed = [tensor[-16:].detach() for tensor in image_enc_pred]
                    reconst = autoencoder.decode(image_enc_pred_trimmed, st=3)
                    orig = image[-16:]
                    # if isinstance(reconst, Image.Image):
                    #     reconst_ = np.array(reconst)
                    # if isinstance(orig, Image.Image):
                    #     orig_ = np.array(orig)

                    # print("Reconst shape:", reconst_.shape) #Reconst shape: (1024, 4096, 3)
                    # print("Orig shape:", orig.shape) # torch.Size([16, 3, 512, 512])

                orig_grid = make_grid((orig * 255).byte(), nrow=int(len(orig) ** 0.5)).permute(1, 2, 0).cpu().numpy()
                orig_grid = Image.fromarray(orig_grid)
                ...
                def resize_to_same_height(img1, img2):
                    ...
                    w1 = int(img1.size[0] * (height / img1.size[1]))
                    w2 = int(img2.size[0] * (height / img2.size[1]))
                    img1 = img1.resize((w1, height), Image.LANCZOS)
                    img2 = img2.resize((w2, height), Image.LANCZOS)
                    return img1, img2

                orig_grid, reconst = resize_to_same_height(orig_grid, reconst)
                comb_grid = np.concatenate([np.array(orig_grid), np.array(reconst)], axis=1)

                image_pil = Image.fromarray(comb_grid)
                image_pil.save(f'{outdir}/reconst_epoch{(epoch + 1):03d}.png')
        ...
        #     wandb.log({
        #         "val/reconstruction": wandb.Image(image_pil, caption=f"Epoch {epoch+1}")
        #     }, step=epoch)
