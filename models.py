...
class Voxel2StableDiffusionModel(torch.nn.Module):
    def __init__(self, in_dim=15724, h=4096, n_blocks=4, use_cont=False, ups_mode='4x', st=5):
        super().__init__()
        self.scales = (1, 2, 3, 4, 6, 9, 13, 18, 24, 32)
        self.st = st
        self.scales = self.scales[:(st+1)]
        ...
        if ups_mode=='4x':
            self.lin1 = nn.Linear(h, 16384, bias=False) # 4*64*64=64*16*16=16384; =256*8*8
            self.norm = nn.GroupNorm(1, 256) #64
    ...
    def forward(self, x, return_transformer_feats=False):
        # print("input x:", x.min().item(), x.max().item(), x.mean().item(), torch.isnan(x).any().item())
        x = self.lin0(x)
        residual = x
        for res_block in self.mlp:
            x = res_block(x)
            # print("res_block x:", x.min().item(), x.max().item(), x.mean().item(), torch.isnan(x).any().item())
            x = x + residual
            residual = x
        x = x.reshape(len(x), -1)
        # bs, 4096
        ...
        # decoder
        # x_16 = self.norm(x.reshape(x.shape[0], -1, side, side).contiguous()) #64*16*16

        # x = self.norm2(x_16.reshape(x_16.shape[0], -1, 8, 8).contiguous()) #256*8*8

        x = self.norm(x.reshape(x.shape[0], -1, side, side).contiguous()) #256*8*8/1024*4*4

        # generate multiple scales
        out_list = []
        for i in range(self.st + 1):
            scale = self.scales[i]
            decoded = self.decoders[i](x) # bs, 32, 32, 32/ bs, 32, 16, 16
            scaled_x = F.interpolate(decoded, size=(scale, scale), mode='bilinear', align_corners=False)
            out_list.append(scaled_x)
        if return_transformer_feats:
        #     return self.upsampler(x), self.maps_projector(x).flatten(2).permute(0,2,1)
        # return self.upsampler(x)
            return out_list, self.maps_projector(x).flatten(2).permute(0,2,1)
        return out_list
